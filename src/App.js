...
function App() {
  ...
  const addItem = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${config.BASE_URL}/api/item`, {
        item: itemText,
      });
  ...
  useEffect(() => {
    const getItemsList = async () => {
      try {
        const res = await axios.get(`${config.BASE_URL}/api/item`);
  ...
  const deleteItem = async (id) => {
    try {
      const res = await axios.delete(`${config.BASE_URL}/api/item/${id}`);
  ...
  const updateItem = async (e) => {
    ...
    try {
      const res = await axios.put(
        `$config.BASE_URL}/api/item/${isUpdating}`,
        { item: updateItemText }
      );
